...
{
	AC_AUTOMATA_t * thiz = (AC_AUTOMATA_t *)AC_MALLOC(sizeof(AC_AUTOMATA_t));
	memset (thiz, 0, sizeof(AC_AUTOMATA_t));
	thiz->root = node_create ();
	thiz->all_nodes_max = REALLOC_CHUNK_ALLNODES;
	thiz->all_nodes = (AC_NODE_t **) AC_MALLOC (thiz->all_nodes_max*sizeof(AC_NODE_t *));
